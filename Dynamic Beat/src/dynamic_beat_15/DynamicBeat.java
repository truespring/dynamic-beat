...
//	선언해준 변수들은 이미지 아이콘과 버튼을 그려주기 위함이다
...
//	이미지에 버튼을 할당한다
	...
	public DynamicBeat() {
		trackList.add(new Track("On my way Title Image.png", "On my way Start Image.png", "On my way Game Image.jpg",
				"Alan Walker - On My Way HL.mp3", "Alan Walker - On My Way.mp3", "On my way"));
		trackList.add(new Track("Faded Title Image.png", "Faded Start Image.png", "Faded Game Image.jpg",
				"Alan Walker - Faded HL.mp3", "Alan Walker - Faded.mp3", "Faded"));
		trackList.add(new Track("The Spectre Title Image.png", "The Spectre Start Image.png",
				"The Spectre Game Image.jpg", "Alan Walker - The Spectre HL.mp3", "Alan Walker - The Spectre.mp3", "The Spectre"));
//	세분화 된 음악 겍체를 리스트에 담아 관리한다 이 부분을 먼저 실행하여 혹여나 발생 할 에러를 대비한다
		setUndecorated(true); // 기본적으로 존재하는 메뉴바가 사라진다
		setTitle("Dynamic Beat"); // 제목을 표시한다
		...
		exitButton.addMouseListener(new MouseAdapter() { // 해당 버튼에 대한 판정 등을 설정한다 
			@Override
			public void mouseEntered(MouseEvent e) { // 마우스가 버튼에 진입했을 때
				exitButton.setIcon(exitButtonEnteredImage);
				exitButton.setCursor(new Cursor(Cursor.HAND_CURSOR)); // 마우스의 커서 모양이 바뀜
			...
			public void mouseExited(MouseEvent e) { // 마우스가 버튼에서 벗어났을 때
				...
				exitButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR)); // 마우스 커서 모양이 원상태로 돌아옴
		...
		leftButton.setVisible(false); // 인트로 화면에서는 보이면 안되는 버튼을 보이지 않게 한다
...
//		메뉴창을 마우스로 클릭한 뒤 움직이는 작업
	...
	public void paint(Graphics g) { // GUI화면에서 가장 첫 번째로 이미지를 그려주는 메소드
		...
		screenDraw((Graphics2D) screenGraphic); // 스크린 이미지에 우리가 원하는 이미지를 그려준다
	...
	public void screenDraw(Graphics2D g) {
		...
		if (isMainScreen) {
			g.drawImage(selectedImage, 340, 100, null); // 곡 이미지와 곡 타이틀을 노출 시킨다
	...
	public void selectTrack(int nowSelected) { // 선택 화면에서 해당하는 요소들을 실행하는 메소드
...
//	게임에 돌입했을 때 사용하는 메소드
	public void gameStart(int nowSelected, String difficulty) {
		if(selectedMusic != null) {
			selectedMusic.close(); // 선택 화면에서 출력되는 노래(HL)를 종료시킨다
		}
		isMainScreen = false;
		leftButton.setVisible(false);
		rightButton.setVisible(false);
		easyButton.setVisible(false);
		hardButton.setVisible(false); // 화면과 각종 버튼을 사라지게 한다
		background = new ImageIcon(Main.class.getResource("../images/" + trackList.get(nowSelected).getGameImage())).getImage();
		backButton.setVisible(true);
		isGameScreen = true; // 게임에 돌입했을 때 필요한 요소들을 출력시킨다
		game = new Game(trackList.get(nowSelected).getTitleName(), difficulty, trackList.get(nowSelected).getGameMusic());
		game.start();
		setFocusable(true);
	}
//	게임 중 뒤로가기 버튼을 눌렀을 때 발생하는 이벤트 메소드
	public void backMain() {
		isMainScreen = true;
		leftButton.setVisible(true);
		rightButton.setVisible(true);
		easyButton.setVisible(true);
		hardButton.setVisible(true); // 각종 버튼을 보이게 함
...
//	인트로 화면에서 노래 선택 화면으로 넘어갔을 때 이벤트 메소드
